...
class Column(pygame.sprite.Sprite):
    def __init__(self, *groups):
        ...
        self.pipe_bottom_rect = self.pipe_bottom.get_rect(
            bottomleft=(0, self.sprite_rect.height + self.sprite_rect.height + self.gap))

        self.pipe_top = pygame.transform.flip(self.sprite, False, True)
        self.pipe_top_rect = self.pipe_top.get_rect(
            topleft=(0, (self.sprite_rect.height - self.sprite_rect.height - self.gap)))
        ...
        sprite_floor_height = assets.get_sprite("base").get_rect().height
        min_y = 100
    ...
    def is_passed(self):
        if self.rect.x < 10 and not self.passed:
            print("passou +1")
            self.passed = True
            return True
        return False
